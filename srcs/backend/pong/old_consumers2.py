...
class GameConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.fetch_game_init_data_from_redis()
    ...
    async def fetch_game_init_data_from_redis(self):
            game_state_key = f"game_state:{self.room_name}"
            game_data = await self.redis.hgetall(game_state_key)

            # Check if game data is successfully fetched
            if game_data:
                print(f"Successfully fetched game data for {self.room_name}: {game_data}")
            else:
                print(f"No game data found for {self.room_name}. Initializing defaults...")

            # Deserialize JSON stored fields
            self.left_paddle = json.loads(game_data.get("leftPaddle", "{}"))
            self.right_paddle = json.loads(game_data.get("rightPaddle", "{}"))
            self.ball = json.loads(game_data.get("ball", "{}"))

            # Assign other data directly
            self.paddle_width = int(game_data["paddleWidth"])
            self.paddle_height = int(game_data["paddleHeight"])
            self.ball_size = int(game_data["ballSize"])
            self.canvas_height = int(game_data["canvasHeight"])
            self.canvas_width = int(game_data["canvasWidth"])
            self.left_player_score = int(game_data["leftPlayerScore"])
            self.right_player_score = int(game_data["rightPlayerScore"])
            self.score_limit = int(game_data["scoreLimit"])
            self.game_status = game_data["gameStatus"]

            # Set ready to play based on status
            self.ready_to_play = 2 if self.game_status in ["ongoing", "paused"] else 0
